...
void UAuraAbilitySystemLibrary::GiveStartupAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ECharacterClass CharacterClass)
{
	...
	for (const auto AbilityClass : CharacterClassInfo->CommAbilities)
	{
	...
	ECharacterClass CharacterClass, int32 CharacterLevel)
{
	...
	const float XPReward = Info.XPReword.GetValueAtLevel(CharacterLevel);
...
bool UAuraAbilitySystemLibrary::IsCritialHit(const FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
...
FGameplayTag UAuraAbilitySystemLibrary::GetDebuffType(const FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
...
void UAuraAbilitySystemLibrary::SetIsCritialHit(FGameplayEffectContextHandle& EffectContextHandle, bool bInIsCritialHit)
{
	if (FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraGameplayEffectContext->SetIsCriticalHit(bInIsCritialHit);
	}
}
	...
	bool bInSuccessfulDebuff)
{
	if (FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraGameplayEffectContext->SetIsDebuffSuccessful(bInSuccessfulDebuff);
	}
}

...
void UExecCalc_Damage::DetermineDebuff(const FGameplayEffectCustomExecutionParameters& ExecutionParams, const FGameplayEffectSpec& Spec, FAggregatorEvaluateParameters EvaluationParameters, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -0.5f) // .5 padding for floating point imprecision
		{
			...
			const FGameplayTag& ResistenceTag = GameplayTags.DamageTypesToResistances[DamageType];
			ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(InTagsToDefs[ResistenceTag], EvaluationParameters, TargetDebuffResistance);
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (auto& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		//if DamageTypeValue is less than or equal to 0, skip the rest of the loop and move on to the next DamageType.  
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 4. Call UGameplayStatics::ApplyRadialDamageWithFalloff in the Lambda, and pass the original damage as BaseDamage, and the damage after resistance as MinimumDamage. This allows to cause damage (this will result in TakeDamage being called
			// on the victim) which will then broadcast OnDamageDelegate
			// 5. In Lambda. set DamageTypeValue to the damage received form the broadcast
	...
	UAuraAbilitySystemLibrary::SetIsCritialHit(EffectContextHandle, bCriticalHit);

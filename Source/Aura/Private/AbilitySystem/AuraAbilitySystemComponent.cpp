...
void UAuraAbilitySystemComponent::ServerUpgradeAttribute_Implementation(const FGameplayTag& AttributeTag)
{
	...
	if (GetAvatarActor()->Implements<UPlayerInterface>())
	{
		//substruct the attribute point, because it is comsumed by player
	...
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			if (!SlotIsEmpty(Slot)) //There is an ability in this slot already, Deactivate and clear its slot
			{
				...
				if (SpecWithSlot)
				{
					// is that ability the same as the one we are trying to equip? If so, we can skip deactivating and reactivating it, just update the slot tag
					...
					{
						//ClientEquipAbility(GetAbilityTagFromSpec(*SpecWithSlot), GameplayTags.Abilities_Status_Unlocked, FGameplayTag(), Slot);
